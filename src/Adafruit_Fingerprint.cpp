...
void uart_flush_tx2(USART_TypeDef *USARTx) {
  ...
  // Optional: clear TC (it's write-clear)
  USARTx->ICR |= USART_ICR_TCCF;
}
  /*uart_flush_tx2(USART1);                                                      \*/
...
*/
/**************************************************************************/
Adafruit_Fingerprint::Adafruit_Fingerprint(USART_TypeDef *huart,
                                           uint32_t password) {
...
void Adafruit_Fingerprint::begin(uint32_t baudrate) {
  //USART_Delay(1000000); // one second delay to let the sensor 'boot up'
...
void Adafruit_Fingerprint::uart_flush_tx(USART_TypeDef *USARTx) {
  // Flush stray RX 
  while (USARTx->ISR & USART_ISR_RXNE) {
    volatile char junk = USARTx -> RDR;
  }

  // Clear err flags
  USARTx->ICR |= USART_ICR_ORECF | USART_ICR_FECF | USART_ICR_PECF;
    ...
    const Adafruit_Fingerprint_Packet &packet) 
{

  {
    ...
    serial_write(USART1, (const char *)buffer, 2);
  }
  ...
  serial_write(USART1, (const char *)packet.address, 4);


  // HAL_UART_Transmit(mySerial, &packet.type, 	  1, TRANSMIT_TIMEOUT);
  serial_write(USART1, (const char *)&packet.type, 1);
  ...
  uart_flush_tx(USART1);
  for (int i = 0; i < packet.length; i++) {
	//HAL_UART_Transmit(mySerial, &packet.data[i], 1, TRANSMIT_TIMEOUT);
    serial_write(USART1, (const char *)&packet.data[i], 1);
    sum += packet.data[i];
#ifdef FINGERPRINT_DEBUG
...
Adafruit_Fingerprint::getStructuredPacket(Adafruit_Fingerprint_Packet *packet,
  ...
  //uart_flush_tx(USART1);
  for (int i = 0; i < 300; i++) {
    buf[i] = serial_read(USART1);
  }  

  
  // serial_write(USART2, (const char*)buf, 300);
...
void Adafruit_Fingerprint::debug_println(const char *format, ...) {
  ...
  va_list args;
  va_start(args, format);
  vsnprintf(buffer, sizeof(buffer) - 3, format, args); // Leave room for \r\n and null-terminator
  va_end(args);
  ...
  if (len < sizeof(buffer) - 2) {
    buffer[len++] = '\r';
    buffer[len++] = '\n';
    buffer[len] = '\0'; // Null-terminate for safety (optional with serial_write)
  }

  serial_write(USART2, buffer, len);
}
