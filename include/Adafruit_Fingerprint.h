...
//#endif
  Adafruit_Fingerprint(USART_TypeDef *huart, uint32_t password = 0x0);
  ...
  void uart_flush_tx(USART_TypeDef *USARTx);
  ...
  uint8_t receiveBytes(uint8_t *data, uint16_t *received_length, uint16_t timeout);
...
private:
  ...
  USART_TypeDef  *mySerial;
  USART_TypeDef  *hwSerial;
  //USARTClass *hwSerial;
};
